...
if [ "${HOST_CHECK}" == "mu" ]; then
    ...
    module load git
elif [ "${HOST_CHECK}" == "pn.lanl.gov" ]; then # Local system
	export PATH="/Users/l302184/Downloads/boost_1_60_0:/opt/local/bin:/opt/local/sbin://anaconda/bin:/Volumes/Macintosh\\ HD/anaconda/bin:/opt/local/bin:/opt/local/sbin:/usr/local/bin:/usr/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/opt/X11/bin:/Library/Globus/bin:/Library/Globus/sbin:/Library/TeX/texbin:${PATH}"
	export GIT_PS1_SHOWUNTRACKEDFILES=t
	export PIO="/usr/local"
	export PNETCDF="/usr/local"
	export PNETCDF_PATH="/usr/local"
	export LD_LIBRARY_PATH="/Users/l302184/Downloads/boost_1_60_0/stage/lib:${LD_LIBRARY_PATH}"
